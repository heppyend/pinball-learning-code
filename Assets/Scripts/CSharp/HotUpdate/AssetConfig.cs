...
{
    public const string Game_Name = "Hall";
    public const string Version = "1.0.0.0";
    public const string Bundle_PostFix = ".bundle";
    public const string Lua_Src_Path = "Assets/Script_HotUpdate/Lua";
    ...
    public Dictionary<string, FileNode> newFileListMap = new Dictionary<string, FileNode>();
    ...
    public void SetPathParam(string url_origin, string dataDir, bool isMainProject)
    {
        ...
        if (_updatePlatform == UpdateType.CurrentPlatform)
        {
            ...
            streamingAssetsPath = Application.dataPath + "/../StreamingAssets" + dataDir + "/Android/";
#elif UNITY_IOS
            url = url_origin + "iOS/";
            streamingAssetsPath = Application.dataPath + "/../StreamingAssets" + dataDir + "/iOS/";
#else
            url = url_origin + "Win/";
            streamingAssetsPath = Application.dataPath + "/../StreamingAssets" + dataDir + "/Win/";
#endif
        }
        else if (_updatePlatform == UpdateType.Android)
        {
...
#elif UNITY_ANDROID
        url = url_origin + "Android/";
        streamingAssetsPath =  Application.streamingAssetsPath + dataDir + "/";
        streamingAssetsPath_Unity =  Application.dataPath + "!assets" + dataDir + "/";
#elif UNITY_IOS
        url = url_origin + "iOS/";
        streamingAssetsPath = Application.streamingAssetsPath + dataDir + "/";
#else
    ...
    {
       // if("room5_98807fab0f6836a870c8697544923307.bundle" == asset)
       // Debug.LogError("IsEditorAssets?" + IsEditorAssets + ",IsUpdate:" + IsUpdate);
        ...
        string path = null;
        // if ("room5_98807fab0f6836a870c8697544923307.bundle" == asset)
       // Debug.LogError("GetPath," + "," + asset + ",IsEditorAssets:" + IsEditorAssets + ",IsUpdate:" + IsUpdate + "," + IsFileExistInStreamingAssets(asset));
        if (IsUpdate)
        {
            path = StringUtil.Concat(persistentDataPath, asset);

           // if ("room5_98807fab0f6836a870c8697544923307.bundle" == asset)
           //     Debug.LogError("GetPath,path," + path);
            if (File.Exists(path))
            {
              //  if ("room5_98807fab0f6836a870c8697544923307.bundle" == asset)
              //      Debug.LogError("存在GetPath," + path + ",IsEditorAssets:" + IsEditorAssets + ",IsUpdate:" + IsUpdate + "," + IsFileExistInStreamingAssets(asset));
                return path;
            }
        }
