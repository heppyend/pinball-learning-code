...
public class AudioManager : DDOLSingleton<AudioManager>
{
    ...
    public AudioSource PlaySoundEff2D(string eff, bool loop = false, float volume = 1)
    {
        AudioSource ret = null;
        //if (!eff.StartsWith("Room"))
        //    return ret;
        //if(eff != "AudioClip/jinbihuishou")
        //    Debug.LogError("声音:" + eff);

        var data = GetClipByName(eff, true);
        if (ReferenceEquals(null, data)) return ret;
        //AudioClip clip = GetClipByName(eff, true);
        //float vol = volume * (0.6f + data.Volume * 0.02f * 0.2f);
        ret = PlaySoundEff2D(data.Clip, loop, 0.6f + 0.2f * data.Volume * 0.02f);
        return ret;
    }

    public void StopSoundEff2D(string eff)
    {
        //if (!eff.StartsWith("Room"))
        //    return;
        Debug.LogError("stop声音:" + eff);
        var data = GetClipByName(eff, true);
        if (ReferenceEquals(null, data)) return;

        StopSoundEff(data.Clip);
    }
    ...
    /// <param name="clip">音频</param>
    /// <param name="loop">是否循环</param>
    public AudioSource PlaySoundEff2D(AudioClip clip, bool loop = false, float volume = 1)
    {
        if (clip == null)
        {
            //Debug.LogError("播放2d音效:返回" );
            return null;
        }

       // Debug.LogError("播放2d音效:" + clip.name);
    ...
    public AudioData GetClipByName(string path, bool cache)
    {
        ...
        //if (obj)
        {
            ...
            data.Clip = ResManager.Instance.LoadAudioClip(path, ".wav");
            data.Volume = 100;
            ...
            // else
            // {
            //     if(path.Contains("Die_Big"))
            //         data.Clip = ResManager.Instance.LoadAudioClip(path, ".wav");
            //     else
            //         data.Clip = ResManager.Instance.LoadAudioClip(path, ".wav");
            //
            //     if (path.Contains("jinbihuishou"))
            //     {
            //         data.Volume = 5;
            //         Debug.LogError("小音量。");
            //     }
