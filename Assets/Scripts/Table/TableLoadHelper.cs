...
public static class TableLoadHelper
{
    ...
    public static void LoadFromLocalPackage()
    {
        ...
        string path = $"{Application.persistentDataPath}/{LocalPackageFileName}";
        if (File.Exists(path))
        {
    ...
    {
        Debug.LogWarning("ossUrl:" + ossUrl);
    ...
    private static async void _asyncLoadFromNet()
    {
        ...
        var savePath = $"{Application.persistentDataPath}/{LocalPackageFileName}";

        // Debug.LogError("_asyncLoadFromNet");
    ...
    public static void _loadJsonImpl(Dictionary<string, string> vFiles, bool onlyLoadFromNet)
    {
        foreach (var t in config.TableHelperTypeList)
        {
            ...
            // Debug.Log(tableName + vFiles[tableName]);
            ...
            if (fileContent == null)
            {
                var localAsset = Resources.Load<TextAsset>(config.LocalJsonDirectory + tableName);
                if (localAsset == null)
                {
                    // WebGL 首次启动可能尚未落盘 config.pkg，允许本地演示继续运行。
                    // 为该表初始化空集合，避免 Resources.Load 结果为空导致首屏 NRE。
                    Debug.LogWarning($"本地配置表缺失，使用空表继续：{tableName}");
                    t.GetMethod("LoadData")?.Invoke(null, new object[] { new List<object>() });
                    continue;
                }

                fileContent = localAsset.text;
            }
    ...
    private static object _createObjectByJToken(JToken token, Type dataType)
    {
        ...
        foreach (var prop in map.PropertiesMap.Values)
        {
            ...
            {
                Debug.LogError($"配置表解析出错：{ex.ToString()}");
            }
        }
    ...
    private static void _setObjectPropertyValue(object obj, TableFieldProperty prop, JToken token)
    {
        ...
        {
            val = token.Type == JTokenType.String ?
                JsonConvert.DeserializeObject(token.ToObject<string>()) as JArray :
                token as JArray;
        }
