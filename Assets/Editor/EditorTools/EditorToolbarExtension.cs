...
namespace UGF.EditorTools
{
    public class EditorToolbarExtension
    {
        ...
        public static readonly string ScenePath = "Assets/HotUpdate/Scenes";
        ...
        static void DrawSwithSceneDropdownMenus()
        {
            ...
            var sceneGuids = AssetDatabase.FindAssets("t:Scene", new string[] { ScenePath });
            ...
            for (int i = 0; i < sceneGuids.Length; i++)
            {
                ...
                bool isInRootDir = Utility.Path.GetRegularPath(ScenePath).TrimEnd('/') == Utility.Path.GetRegularPath(fileDir).TrimEnd('/');
                var sceneName = System.IO.Path.GetFileNameWithoutExtension(scenePath);
                string displayName = sceneName;
                if (!isInRootDir)
                {
                    var sceneDir = System.IO.Path.GetRelativePath(ScenePath, fileDir);
