...
namespace YooAsset
{
    ...
    //{
    //    // 方法也改为internal（或保持public，但通常与类的访问级别一致）
    //    public static T[] AbandonVSAdopt<T>(ref int id, IEnumerable<T> sourceList, Func<T, int> idSelector)
    //    {
    //        int tempId = id;
    //        var filteredList = sourceList.Where(t => idSelector(t) % 100 == tempId).ToArray();
    //        id++;
    //        return filteredList;
    //    }
    //}
